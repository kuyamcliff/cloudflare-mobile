    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = viewModel::openForm) {
                Icon(Icons.Filled.Add, contentDescription = "Create tunnel")
            }
        }
    ) { padding ->
        StateContent(state = uiState.tunnels, onRetry = viewModel::refresh) { tunnels ->
            if (tunnels.isEmpty()) {
                EmptyState("No tunnels yet", Modifier.padding(padding))
            } else {
                LazyColumn(Modifier.padding(padding), contentPadding = PaddingValues(bottom = 96.dp)) {
                    items(tunnels, key = { it.id }) { tunnel ->
                        TunnelRow(tunnel, isDeleting = uiState.deletingId == tunnel.id, onDelete = { viewModel.delete(tunnel) })
...
@Composable
private fun TunnelRow(tunnel: CfTunnel, isDeleting: Boolean, onDelete: () -> Unit) {
    ...
    Row(
        ...
        Column(Modifier.weight(1f)) {
            ...
            tunnel.status?.let {
                Text(it.replaceFirstChar { c -> c.uppercase() }, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
        }
        if (isDeleting) {
            ...
            IconButton(onClick = { confirmDelete = true }) {
                Icon(Icons.Filled.Delete, contentDescription = "Delete tunnel", tint = MaterialTheme.colorScheme.error)
            }
        }
    }
    if (confirmDelete) {
        AlertDialog(
            ...
            text = { Text("\"${tunnel.name}\" will be permanently deleted. This can't be undone.") },
